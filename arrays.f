...
C FIRST AND LAST ARE FOR CURRENT TASK, PIVOT = THE PIVOT
      INTEGER TAP, TEP, FIRST, LAST, PIVOT
      TASKS(1, 1) = 1
      TASKS(1, 2) = SIZE
      TAP = 2
      TEP = 0
...
C TO DO
      IF (FIRST .EQ. LAST) GOTO 200
C THE FOLLOWING IS BASED ON THE DEFINITION OF PARTITION ON PAGE 154 OF
C THE BIG BOOK OF ALGORITHMS
210   PIVOT = ARRAY(FIRST)
C PARTITION THE ARRAY SO THINGS LESS THAN THE PIVOT COME FIRST, THEN
C THE PIVOT, THEN THINGS GREATER THAN THE PIVOT
      I = FIRST - 1
      J = LAST + 1
220   J = J - 1
      IF (ARRAY(J) .GT. PIVOT) GOTO 220 
C FOUND AT LEAST ONE OTHER VALUE
230   I = I + 1
      IF (ARRAY(I) .LT. PIVOT) GOTO 230
C I IS NOW POINTING AT A VALUE GREATER OR EQUAL TO THE PIVOT, AND J IS 
C POINTING AT A VALUE LESS THAN OR EQUAL TO THE PIVOT, SO SWAP THEM
      IF (I .GE. J) GOTO 240
      M = ARRAY(I)
      ARRAY(I) = ARRAY(J)
      ARRAY(J) = M
      GOTO 220
240   TASKS(TAP, 1) = FIRST
      TASKS(TAP, 2) = J
      TAP = TAP + 1
      TASKS(TAP, 1) = J+1
      TASKS(TAP, 2) = LAST
